...
@pytest.mark.parametrize("sample_rate,expected_decision", [(0.0, False), (1.0, True)])
def test_uses_all_or_nothing_traces_sample_rate_correctly(
    sentry_init, sample_rate, expected_decision
):
    sentry_init(traces_sample_rate=sample_rate)

    transaction = start_transaction(name="dogpark")
    assert transaction.sampled == expected_decision


@pytest.mark.parametrize(
    "random_value,expected_decision", [(0.25, True), (0.75, False)]
)
def test_uses_intermediate_traces_sample_rate_correctly(
    sentry_init, random_value, expected_decision
):
    sentry_init(traces_sample_rate=0.5)

    with mock.patch.object(random, "random", return_value=random_value):
        transaction = start_transaction(name="dogpark")
...
def test_tolerates_traces_sampler_returning_a_boolean(
    sentry_init, FunctionMock, traces_sampler_return_value
):
...
def test_only_captures_transaction_when_sampled_is_true(
    sentry_init, FunctionMock, sampling_decision, capture_events
):
...
def test_prefers_traces_sampler_to_traces_sample_rate(
    sentry_init, FunctionMock, traces_sample_rate, traces_sampler_return_value
):
...
def test_warns_and_sets_sampled_to_false_on_invalid_traces_sampler_return_value(
    sentry_init, traces_sampler_return_value, StringContaining
):
